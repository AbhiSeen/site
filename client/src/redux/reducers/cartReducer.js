...
export const cartReducer = (state = { cartItems: []}, action) => {
    ...
    switch(action.type) {
        case actionTypes.ADD_TO_CART:{    
            ...
            if(existItem){    
                newCartItems={...state,cartItems: state.cartItems.map(x => x._id===item._id ?  {...x,quantity:x.quantity+1} : x)} 
            } else {               
        ...
        case actionTypes.REMOVE_FROM_CART:{
            newCartItems={...state,cartItems:state.cartItems.filter(product => product._id !== action.payload)};
            localStorage.setItem("cartItem",JSON.stringify(newCartItems));
            return newCartItems;         
        }
